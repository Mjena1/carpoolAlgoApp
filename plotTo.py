...
def plot(locations, assignments, driver_paths):
    ...
    for driver, companion_list in assignments.items():
        for companion, meeting_point in companion_list:
            ...
            folium.Marker(
                ...
                tooltip=f"{companion}'s Location",
                icon=folium.Icon(color='green', icon='user', prefix='fa')
            ).add_to(companion_cluster)

            folium.Marker(
                meeting_point,
                popup=f"Meeting Point for {companion}",
    ...
    ">
        ...
        <i class="fa fa-user fa-1x" style="color:green"></i> Companion<br>
        <i class="fa fa-flag fa-1x" style="color:orange"></i> Meeting Point<br>
        <i class="fa fa-car fa-1x" style="color:blue"></i> Driver Start<br>
    </div>
    {% endmacro %}
    """
    legend = MacroElement()
